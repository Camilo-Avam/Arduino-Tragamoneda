...
void reconnect() {
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(mqtt_clientID, "camilo", "12345678", mqtt_topic_ping, 0, false, mqtt_last_msg, false)) {
...
void callbackMQTT(char* topic, byte* payload, unsigned int length) {
  ...
  if (SERIAL_ID == serial) {
    ...
    if (action == "COLLECT") {
      ...
      delay(50);
      digitalWrite(pinSalida, LOW);
      mqttClient.publish(mqtt_topic, "KeyOut enviado a la tarjeta ");
    }
    if (action == "RESET") {
      Serial.println(F("Reinicio"));
      String informacion = "{\"reset\": \"" + actualizaFechaHora() + "\"},";
      saveSD("/reset.txt", informacion);
      delay(100);
      mqttClient.publish(mqtt_topic, "Reinicio enviado a tarjeta");
      esp_restart();
    }
    if (action == "GET_FILE") {
      ...
      if (file) {
        mqttClient.publish(mqtt_topic, "Archivo: ");
        file.close();
      } else {
        mqttClient.publish(mqtt_topic, "Archivo no encontrado ");
      }
    }
    if (action == "ping_response") {
      conecto_mqtt = true;
      mqttClient.publish(mqtt_topic_ping, "Conectado con mqtt backend ");
    }
