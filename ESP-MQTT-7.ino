...
const String SERIAL_ID = "777";  // Serial de la maquina´
...
void sendJson(String valor, String endPoint) {  // sendHttpPOST() Envia la peticion POST al dashboard
  ...
  sendHTTP(endPoint, jsonString, jsonString2);
}
...
//---------------------------------------------------------------------------------------------------------
void sendHTTP(String endPoint, String json, String json2) {
  ...
  const char* message = json2.c_str();
    ...
    while (central.connected()) {
      ...
      if (characteristicBLE.written()) {
        ...
        if (characteristicBLE.value() == "reset") {
          ...
          characteristicBLE.writeValue("RESETOK");
          delay(4000);
          esp_restart();
        }
      }
