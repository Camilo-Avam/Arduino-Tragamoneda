...
BLEStringCharacteristic characteristicBLE_casIn("9d0983d4-0c64-45c4-b83b-14abaa6ba18b", BLERead | BLEWrite, 1024);
...
const String SERIAL_ID = "CG-601-001";  // Serial de la maquina´
  ...
  if (central) {
    ...
    while (central.connected()) {
      ...
      if (characteristicBLE.written()) {
        Serial.println("Ingreso en el servicio");
        if (characteristicBLE.value() == "cashOUT") {  // any value other than 0
          Serial.println("Ingreso en el cash out");
        ...
        if (characteristicBLE.value() == "ingreso") {  // any value other than 0
      ...
      if (characteristicBLE_casIn.written()) {
        if (characteristicBLE_casIn.value() == "ingreso") {  // any value other than 0
          String contenido = readSD("/cashIN.txt", "todo");
          Serial.println(contenido);
          characteristicBLE_casIn.writeValue(contenido);
        }
      }
...
void setup() {
  ...
  serviceBLE.addCharacteristic(characteristicBLE_casIn);
  ...
  characteristicBLE_casIn.writeValue("");
