...
char fileContent[64]; // Variable para almacenar el contenido del archivo
char ssid[32]; // Variables para almacenar el SSID y la contraseña
char password[32];
WebServer server(80);
...
void setup() {
  ...
  File myFile = SD.open("/claves.txt", FILE_READ);
  if (myFile) {
    int i = 0;
  ...
  if (token != NULL) {
    ...
    if (token != NULL) {
      ...
      Serial.println("No se encontró la contraseña en el archivo.");
    }
  } else {
    Serial.println("No se encontró el SSID en el archivo.");
  }
  // Lectura WIFI
  WiFi.mode(WIFI_STA);
