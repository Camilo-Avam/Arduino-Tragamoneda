...
void sendHTTP(String endPoint, String json) {
  ...
  Serial.println(json);
  Serial.println(F("enviar http send"));
...
void syncData() {
  HTTPClient httpSync;
  String urlIn = backendURL + "synchronization/cashInSyn.txt";
  String urlOut = backendURL + "synchronization/cashOutSyn.txt";

  vTaskDelay(pdMS_TO_TICKS(100));

  File fileIn = SD.open("/cashInSyn.txt");

  size_t fileSizeIn = fileIn.size();
  uint8_t* bufferIn = (uint8_t*)malloc(fileSizeIn);
  if (!bufferIn) {
    Serial.println("Error al reservar memoria para el archivo");
    fileIn.close();
    return;
  }

  if (fileIn.read(bufferIn, fileSizeIn) != fileSizeIn) {
  ...
  int ResponseCodeIn = httpSync.POST(bufferIn, fileSizeIn);
  if (ResponseCodeIn > 0) {
  ...
  size_t fileSizeOut = fileOut.size();
  uint8_t* bufferOut = (uint8_t*)malloc(fileSizeOut);
  if (!bufferOut) {
    Serial.println("Error al reservar memoria para el archivo Out");
    fileOut.close();
    return;
  }

  if (fileOut.read(bufferOut, fileSizeOut) != fileSizeOut) {
    Serial.println("Error al leer el archivo");
    fileOut.close();
    free(bufferOut);
    return;
  ...
  int ResponseCodeOut = httpSync.POST(bufferOut, fileSizeOut);
